...
aws ecs describe-task-definition --task-definition applications-2 | ./jq --arg x $CIRCLE_SHA1 ' .taskDefinition
                                                                                                | del(.status)
...
aws ecs update-service --cluster alexandria --service faros-2 --task-definition "applications-2:$NEW_REVISION"
